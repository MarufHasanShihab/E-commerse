import p_img1 from './p_img1.png';
import p_img10 from './p_img10.png';
import p_img11 from './p_img11.png';
import p_img12 from './p_img12.png';
import p_img13 from './p_img13.png';
import p_img14 from './p_img14.png';
import p_img15 from './p_img15.png';
import p_img16 from './p_img16.png';
import p_img17 from './p_img17.png';
import p_img18 from './p_img18.png';
import p_img19 from './p_img19.png';
import p_img20 from './p_img20.png';
import p_img21 from './p_img21.png';
import p_img22 from './p_img22.png';
import p_img23 from './p_img23.png';
import p_img24 from './p_img24.png';
import p_img25 from './p_img25.png';
import p_img26 from './p_img26.png';
import p_img27 from './p_img27.png';
import p_img28 from './p_img28.png';
import p_img29 from './p_img29.png';
import p_img2_1 from './p_img2_1.png';
import p_img2_2 from './p_img2_2.png';
import p_img2_3 from './p_img2_3.png';
import p_img2_4 from './p_img2_4.png';
import p_img30 from './p_img30.png';
import p_img31 from './p_img31.png';
import p_img32 from './p_img32.png';
import p_img33 from './p_img33.png';
import p_img34 from './p_img34.png';
import p_img35 from './p_img35.png';
import p_img36 from './p_img36.png';
import p_img37 from './p_img37.png';
import p_img38 from './p_img38.png';
import p_img39 from './p_img39.png';
import p_img4 from './p_img4.png';
import p_img40 from './p_img40.png';
import p_img41 from './p_img41.png';
import p_img42 from './p_img42.png';
import p_img43 from './p_img43.png';
import p_img44 from './p_img44.png';
import p_img45 from './p_img45.png';
import p_img46 from './p_img46.png';
import p_img47 from './p_img47.png';
import p_img48 from './p_img48.png';
import p_img49 from './p_img49.png';
import p_img50 from './p_img50.png';
import p_img51 from './p_img51.png';
import p_img52 from './p_img52.png';
import p_img6 from './p_img6.png';
import p_img7 from './p_img7.png';
import p_img8 from './p_img8.png';

import arrow_icon from './arrow_icon.svg';
import bin_icon from './bin_icon.png';
import cart_icon from './cart_icon.png';
import dropdown_icon from './dropdown_icon.png';
import exchange_icon from './exchange_icon.png';
import hero_img from './hero_img.png';
import profile_icon from './profile_icon.png';
import quality_icon from './quality_icon.png';
import search_icon from './search_icon.png';
// import star_dull_icon from './star_dull_icon.png'
// import star_icon from './star_icon.png'
import about_img from './about_img.png';
import contact_img from './contact_img.png';
import cross_icon from './cross_icon.png';
import header_headphone_image from './header_headphone_image.png';
import header_macbook_image from './header_macbook_image.png';
import header_playstation_image from './header_playstation_image.png';
import heart_icon from './heart_icon.svg';
import menu_icon from './menu_icon.png';
import razorpay_logo from './razorpay_logo.png';
import star_dull_icon from './star_dull_icon.svg';
import star_icon from './star_icon.svg';
import stripe_logo from './stripe_logo.png';
import support_img from './support_img.png';

export const assets = {
  arrow_icon,
  hero_img,
  cart_icon,
  dropdown_icon,
  exchange_icon,
  profile_icon,
  quality_icon,
  search_icon,
  star_dull_icon,
  star_icon,
  bin_icon,
  support_img,
  menu_icon,
  about_img,
  contact_img,
  razorpay_logo,
  stripe_logo,
  cross_icon,
  header_headphone_image,
  header_playstation_image,
  header_macbook_image,
  heart_icon,
};

export const products = [
  {
    _id: 'aaaaa',
    name: 'Women Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 100,
    image: [p_img1],
    category: 'Fashion',
    sizes: ['S', 'M', 'L'],
    date: 1716634345448,
    bestseller: false,
  },
  {
    _id: 'aaaab',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 200,
    image: [p_img2_1, p_img2_2, p_img2_3, p_img2_4],
    category: 'Fashion',
    sizes: ['M', 'L', 'XL'],
    date: 1716621345448,
    bestseller: false,
  },
  {
    _id: 'aaaac',
    name: 'Samsung Galaxy S23',
    description:
      'The Samsung Galaxy S23 offers an all-encompassing mobile experience with its advanced AMOLED display, offering vibrant visuals and smooth interactions. Equipped with top-of-the-line fitness tracking features and cutting-edge technology, this phone delivers outstanding performance. With powerful hardware, a sleek design, and long battery life, the S23 is perfect for those who demand the best in mobile innovation.',
    price: 799.99,
    image: [
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/xjd4eprpwqs7odbera1w.webp',
    ],
    category: 'Gadget',
    date: 1716234545448,
    bestseller: false,
  },
  {
    _id: 'aaaad',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 110,
    image: [p_img4],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'XXL'],
    date: 1716621345448,
    bestseller: true,
  },
  {
    _id: 'aaaae',
    name: 'Apple AirPods Pro 2nd gen',
    description:
      'Apple AirPods Pro (2nd Gen) with MagSafe Case (USB-C) provide excellent sound, active noise cancellation, and a comfortable fit. The USB-C case ensures quick charging, and they pair seamlessly with Apple devices for an effortless audio experience..',
    price: 499.99,
    image: [
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/k4dafzhwhgcn5tnoylrw.webp',
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/j212frakb8hdrhvhajhg.webp',
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/imwuugqxsajuwqpkegb5.webp',
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/k1oqaslw5tb3ebw01vvj.webp',
    ],
    category: 'Gadget',
    date: 1716622345448,
    bestseller: false,
  },
  {
    _id: 'aaaaf',
    name: 'Finger ring -GC5',
    description:
      'This finger ring -GC5 is expertly crafted with high-quality materials and precise design. The sleek and modern style adds a touch of sophistication to any outfit. With its durability and elegance, this finger ring is perfect for everyday wear or a special occasion. Made with comfort and style in mind, -GC5 is a must-have accessory.',
    price: 140,
    image: [p_img6],
    category: 'Jewellery',
    date: 1716623423448,
    bestseller: false,
  },
  {
    _id: 'aaaag',
    name: 'Men Tapered Fit Flat-Front Trousers',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 190,
    image: [p_img7],
    category: 'Fashion',
    sizes: ['S', 'L', 'XL'],
    date: 1716621542448,
    bestseller: false,
  },
  {
    _id: 'aaaah',
    name: 'Makeup bowl',
    description:
      'This makeup bowl includes 9 pcs of beauty tools, perfect for professional and at-home use. The various tools provide convenience and precision for a flawless makeup application. Each tool is carefully crafted to enhance the makeup experience and achieve desired results.',
    price: 140,
    image: [p_img8],
    category: 'Jewellery',
    date: 1716622345448,
    bestseller: false,
  },
  {
    _id: 'aaaai',
    name: 'Canon EOS R5',
    description:
      "The Canon EOS R5 is a game-changing mirrorless camera with a 45MP full-frame sensor, offering ultra-high resolution and the ability to shoot 8K video. Whether you're capturing professional-quality stills or cinematic video footage, this camera delivers exceptional clarity, speed, and color accuracy. With advanced autofocus and in-body stabilization, the R5 is ideal for photographers and videographers alike.",
    price: 100,
    image: [
      'https://raw.githubusercontent.com/avinashdm/gs-images/main/quickcart/r5h370zuujvrw461c6wy.webp',
    ],
    category: 'Gadget',
    date: 1716621235448,
    bestseller: false,
  },
  {
    _id: 'aaaaj',
    name: 'Men Tapered Fit Flat-Front Trousers',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 110,
    image: [p_img10],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'L', 'XL'],
    date: 1716622235448,
    bestseller: false,
  },
  {
    _id: 'aaaak',
    name: 'Pearl Studded Oxidized Silver Bangle',
    description:
      'White pearl studded engraved oxidized silver bangle. Pearl Studded Oxidized Silver Bangle',
    price: 120,
    image: [p_img11],
    category: 'Jewellery',
    date: 1716623345448,
    bestseller: false,
  },
  {
    _id: 'aaaal',
    name: 'Oxidized Silver Necklace Set',
    description:
      'Thread oxidized silver necklace with engraved detailing. Comes with a pair of matching earrings.',
    price: 150,
    image: [p_img12],
    category: 'Jewellery',
    date: 1716624445448,
    bestseller: false,
  },
  {
    _id: 'aaaam',
    name: 'Women Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 130,
    image: [p_img13],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716625545448,
    bestseller: false,
  },
  {
    _id: 'aaaan',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 160,
    image: [p_img14],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716626645448,
    bestseller: false,
  },
  {
    _id: 'aaaao',
    name: 'Men Tapered Fit Flat-Front Trousers',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 140,
    image: [p_img15],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716627745448,
    bestseller: false,
  },
  {
    _id: 'aaaap',
    name: 'Girls Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 170,
    image: [p_img16],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716628845448,
    bestseller: false,
  },
  {
    _id: 'aaaaq',
    name: 'Men Tapered Fit Flat-Front Trousers',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 150,
    image: [p_img17],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716629945448,
    bestseller: false,
  },
  {
    _id: 'aaaar',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 180,
    image: [p_img18],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716631045448,
    bestseller: false,
  },
  {
    _id: 'aaaas',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 160,
    image: [p_img19],
    category: 'Fasion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716632145448,
    bestseller: false,
  },
  {
    _id: 'aaaat',
    name: 'Women Palazzo Pants with Waist Belt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 190,
    image: [p_img20],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716633245448,
    bestseller: true,
  },
  {
    _id: 'aaaau',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 170,
    image: [p_img21],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716634345448,
    bestseller: false,
  },
  {
    _id: 'aaaav',
    name: 'Women Palazzo Pants with Waist Belt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 200,
    image: [p_img22],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716635445448,
    bestseller: false,
  },
  {
    _id: 'aaaaw',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 180,
    image: [p_img23],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716636545448,
    bestseller: false,
  },
  {
    _id: 'aaaax',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 210,
    image: [p_img24],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716637645448,
    bestseller: false,
  },
  {
    _id: 'aaaay',
    name: 'Girls Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 190,
    image: [p_img25],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716638745448,
    bestseller: true,
  },
  {
    _id: 'aaaaz',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 220,
    image: [p_img26],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716639845448,
    bestseller: false,
  },
  {
    _id: 'aaaba',
    name: 'Girls Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 200,
    image: [p_img27],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716640945448,
    bestseller: false,
  },
  {
    _id: 'aaabb',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 230,
    image: [p_img28],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716642045448,
    bestseller: true,
  },
  {
    _id: 'aaabc',
    name: 'Women Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 210,
    image: [p_img29],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716643145448,
    bestseller: false,
  },
  {
    _id: 'aaabd',
    name: 'Girls Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 240,
    image: [p_img30],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716644245448,
    bestseller: false,
  },
  {
    _id: 'aaabe',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 220,
    image: [p_img31],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716645345448,
    bestseller: true,
  },
  {
    _id: 'aaabf',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 250,
    image: [p_img32],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716646445448,
    bestseller: false,
  },
  {
    _id: 'aaabg',
    name: 'Girls Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 230,
    image: [p_img33],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716647545448,
    bestseller: false,
  },
  {
    _id: 'aaabh',
    name: 'Women Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 260,
    image: [p_img34],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716648645448,
    bestseller: false,
  },
  {
    _id: 'aaabi',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 240,
    image: [p_img35],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716649745448,
    bestseller: false,
  },
  {
    _id: 'aaabj',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 270,
    image: [p_img36],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716650845448,
    bestseller: false,
  },
  {
    _id: 'aaabk',
    name: 'Women Round Neck Cotton Top',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 250,
    image: [p_img37],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716651945448,
    bestseller: false,
  },
  {
    _id: 'aaabl',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 280,
    image: [p_img38],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716653045448,
    bestseller: false,
  },
  {
    _id: 'aaabm',
    name: 'Men Printed Plain Cotton Shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 260,
    image: [p_img39],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716654145448,
    bestseller: false,
  },
  {
    _id: 'aaabn',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 290,
    image: [p_img40],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716655245448,
    bestseller: false,
  },
  {
    _id: 'aaabo',
    name: 'Men Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 270,
    image: [p_img41],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716656345448,
    bestseller: false,
  },
  {
    _id: 'aaabp',
    name: 'Boy Round Neck Pure Cotton T-shirt',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 300,
    image: [p_img42],
    category: 'Fashion',
    subCategory: 'Topwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716657445448,
    bestseller: false,
  },
  {
    _id: 'aaabq',
    name: 'Kid Tapered Slim Fit Trouser',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 280,
    image: [p_img43],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716658545448,
    bestseller: false,
  },
  {
    _id: 'aaabr',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 310,
    image: [p_img44],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716659645448,
    bestseller: false,
  },
  {
    _id: 'aaabs',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 290,
    image: [p_img45],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716660745448,
    bestseller: false,
  },
  {
    _id: 'aaabt',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 320,
    image: [p_img46],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716661845448,
    bestseller: false,
  },
  {
    _id: 'aaabu',
    name: 'Kid Tapered Slim Fit Trouser',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 300,
    image: [p_img47],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716662945448,
    bestseller: false,
  },
  {
    _id: 'aaabv',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 330,
    image: [p_img48],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716664045448,
    bestseller: false,
  },
  {
    _id: 'aaabw',
    name: 'Kid Tapered Slim Fit Trouser',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 310,
    image: [p_img49],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716665145448,
    bestseller: false,
  },
  {
    _id: 'aaabx',
    name: 'Kid Tapered Slim Fit Trouser',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 340,
    image: [p_img50],
    category: 'Fashion',
    subCategory: 'Bottomwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716666245448,
    bestseller: false,
  },
  {
    _id: 'aaaby',
    name: 'Women Zip-Front Relaxed Fit Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 320,
    image: [p_img51],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716667345448,
    bestseller: false,
  },
  {
    _id: 'aaabz',
    name: 'Men Slim Fit Relaxed Denim Jacket',
    description:
      'A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.',
    price: 350,
    image: [p_img52],
    category: 'Fashion',
    subCategory: 'Winterwear',
    sizes: ['S', 'M', 'L', 'XL'],
    date: 1716668445448,
    bestseller: false,
  },
];
